% !TEX program = xelatex
% FYP Progress Review — 8 June 2026
...
% ── Metadata ─────────────────────────────────────────────────────────────────
\title[FYP Progress Review]{Vibe Networking --- Network Engineering}
\subtitle{FYP Progress Review}
\author[FYP Student]
       {FYP Student\\[4pt]
...
\begin{frame}{Progress: Agent Architecture}
  ...
  \begin{tikzpicture}[
    ...
    \node[box] (cli) {CLI\\\texttt{fyp-agent run}};
  ...
  \vspace{4pt}
  \begin{center}
    \footnotesize Python 3.13 \quad$\cdot$\quad
    OpenAI Agents SDK 0.6 \quad$\cdot$\quad
    cml-mcp 0.28 + virl2-client 2.9.1
  \end{center}
\end{frame}
...
\begin{frame}{Progress: Agent Demo}
  \textbf{Example Run}: \texttt{fyp-agent run "list all labs on CML"}
