...
public record NetagentSettings(
        ...
        Duration mcpTimeout,
        Path runsRoot,
        Long workbenchExperimentId,
        Long workbenchAgentConfigId
) {
    ...
    public static NetagentSettings fromMap(Map<String, String> env) {
        ...
        return new NetagentSettings(
                ...
                longValue(env, "NETAGENT_WORKBENCH_EXPERIMENT_ID"),
                longValue(env, "NETAGENT_WORKBENCH_AGENT_CONFIG_ID")
        );
    }
    ...
    private static Long longValue(Map<String, String> env, String key) {
        String value = env.get(key);
        if (value == null || value.isBlank()) {
            return null;
        }
